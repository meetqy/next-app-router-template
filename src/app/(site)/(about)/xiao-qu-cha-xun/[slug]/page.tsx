...
export default async function CampusDetailPage({ params }: PageProps) {
	...
	const reviewGroups = reviews.reduce<
		{ screenshot: string; reviews: CampusUserReview[] }[]
	>((groups, review) => {
							...
							<section className="border-border/70 border-t py-7 md:py-8">
								...
								<div className="mt-6 space-y-8">
									{reviewGroups.map((group, index) => (
										<div
											className="flex flex-col-reverse items-start gap-5 md:flex-row"
											key={group.screenshot}
										>
											<div className="min-w-0 flex-1 divide-y divide-border/70">
												{group.reviews.map((review) => (
													<article className="py-6 first:pt-0 last:pb-0" key={`${review.reviewer}-${review.date}`}>
														...
														{review.content ? (
															<p className="mt-4 whitespace-pre-line text-slate-700 leading-8">
																{review.content}
															</p>
														) : (
											...
											<div className="w-full shrink-0 md:w-44 lg:w-52">
												<a
													...
													href={imageUrl(group.screenshot)}
													rel="noopener noreferrer"
													target="_blank"
												>
